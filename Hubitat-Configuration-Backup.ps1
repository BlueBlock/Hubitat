...
# hub location/name
$WebResponseObj = Invoke-WebRequest -Uri "$urlhub/location/edit" -TimeoutSec 15 -ErrorAction:Stop
$location = ($WebResponseObj.Forms | Where {$_.Action -eq "/location/update"}).Fields.name

# hub software version
$WebResponseObj = Invoke-WebRequest -Uri "$urlhub/hub/edit" -TimeoutSec 15 -ErrorAction:Stop
...
$url = "http://hubitat/hub/backupDB?fileName=latest"
...
if ($WhatIf) {
    ...
    Invoke-WebRequest -OutFile $output -Uri $url -TimeoutSec 15 -ErrorAction:Stop
}
